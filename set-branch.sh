#!/bin/bash
# This script sets the current branch name in the .git/HEAD file.
# Usage: ./set-branch.sh <branch-name>
# Example: ./set-branch.sh feature/new-feature
...
# Set the branch name in the .git/HEAD file
echo "ref: refs/heads/$1" > .git/HEAD
# Check if the command was successful
if [ $? -eq 0 ]; then
  echo "Successfully set the current branch to '$1'."
else
  echo "Error: Failed to set the current branch."
  exit 1
fi
# Check if the branch name is valid
